...
In this animation plot we see that...
...
- When log10(malaria) is 0, that is also when malaria cases per 100,000 = 1; the model predicts the rural basic sanitation rate to be approximately 78.92%.

- It represents the expected sanitation level in countries with extremely low malaria burden.

Coefficient

- For every 1-unit increase in log10(malaria) or for every 10 unit increase in malaria cases per 100,000 people. The model predicts an average decrease of 14.73 percentage points in the rural sanitation rate.

Significance

- Because the P-value is essentially 0, it shows us that at a 95% significiance level, we are confident that there is a significant relationship between Malaria Cases per 100,000 and Access to rural basic sanitation.
